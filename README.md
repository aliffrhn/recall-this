...
- **AI summary** – optional OpenAI recap triggered from the transcript view after you click "Get AI summary."
...
export OPENAI_API_KEY=sk-...       # optional; used if no key is pasted in the UI
export OPENAI_SUMMARY_MODEL=gpt-4o-mini
...
4. (Optional) click **OpenAI key setup**, paste your key once, and click **Get AI summary** under the transcript to generate bullet notes. Nothing is stored server-side after refresh.
...
- Without an OpenAI key, the transcript still works; the summary card will prompt the user to add a key.
